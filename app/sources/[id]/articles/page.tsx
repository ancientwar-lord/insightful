...
async function getSourceArticles(id: string) {
  try {
    // Fetch source details directly from the articles response or we can just fetch it, 
    // but our API doesn't have a /sources/{id} endpoint yet. 
    // We can just fetch the articles and use the source_name from the first article.
    const artRes = await fetch(buildApiUrl(`/sources/${id}/articles`), { cache: "no-store" });
...
export default async function SourceArticlesPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = await params;
  const data = await getSourceArticles(resolvedParams.id);
  
  if (!data) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-black">
        <p className="text-xl text-zinc-500">Source not found or failed to load.</p>
      </div>
    );
  }
  ...
  // Extract source name from the first article if available
  const sourceName = articles.length > 0 ? articles[0].source_name : "Source";
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-50 capitalize">
                {sourceName} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-violet-600">News</span>
              </h1>
              <p className="text-zinc-600 dark:text-zinc-400 mt-1">
                Articles tracked from this specific source.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3 shrink-0 self-end md:self-center">
            <DeleteSourceButton id={parseInt(resolvedParams.id)} sourceName={sourceName} redirectTo="/sources" />
          </div>
        </div>
        ...
        <div className="w-full mt-4">
          {articles.length === 0 ? (
            <div className="text-center py-24 bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 border-dashed">
              <p className="text-zinc-500 dark:text-zinc-400 text-lg font-medium">No articles found for this source.</p>
            </div>
          ) : (
