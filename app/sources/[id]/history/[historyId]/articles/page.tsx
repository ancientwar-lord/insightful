...
async function getSourceHistoryArticles(id: string, historyId: string, skip: number = 0, limit: number = 21) {
  try {
    ...
    // Fetch source history articles
    const artRes = await fetch(buildApiUrl(`/sources/${id}/history/${historyId}/articles?skip=${skip}&limit=${limit}`), { cache: "no-store" });
    if (!artRes.ok) throw new Error("Failed to fetch articles");
    const articlesData = await artRes.json();
    
    return { source, articlesData };
  } catch (error) {
    console.error("getSourceHistoryArticles ERROR:", error);
    return null;
  }
}

export default async function SourceHistoryArticlesPage(props: { 
  ...
  const data = await getSourceHistoryArticles(resolvedParams.id, resolvedParams.historyId, skip, limit);
  
  if (!data) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-black">
        <p className="text-xl text-zinc-500">History or Source not found.</p>
      </div>
    );
  }

  const { source, articlesData } = data;
      ...
      <main className="max-w-6xl mx-auto px-6 pt-16 pb-32 flex flex-col items-center">
        ...
        <div className="w-full flex flex-col md:flex-row md:items-center justify-between gap-4 mb-12">
          <div className="flex items-center gap-4">
            <Link href={`/sources/${source.id}/history`} className="text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-50 capitalize">
                {source.name} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-violet-600">Refresh</span>
              </h1>
              <p className="text-zinc-600 dark:text-zinc-400 mt-1">
                Articles freshly saved for this source during this specific refresh.
              </p>
              {articlesData.total > 0 && (
                <p className="text-sm font-medium text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-800/50 inline-block px-4 py-1.5 rounded-full mt-2">
                  Showing {articlesData.total} total news articles
                </p>
              )}
            </div>
          </div>
        ...
        <div className="w-full mt-4">
          {articles.length === 0 ? (
            <div className="text-center py-24 bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 border-dashed">
              <p className="text-zinc-500 dark:text-zinc-400 text-lg font-medium">No new articles were saved during this session.</p>
            </div>
          ) : (
        ...
        {articlesData.total > limit && (
          <div className="mt-16 flex items-center justify-center gap-4">
            {page > 1 ? (
              <Link
                href={`/sources/${source.id}/history/${resolvedParams.historyId}/articles?page=${page - 1}`}
            ...
            <span className="text-sm font-medium text-zinc-600 dark:text-zinc-400">
              Page {page} of {Math.ceil(articlesData.total / limit)}
            </span>
            
            {page * limit < articlesData.total ? (
              <Link
                href={`/sources/${source.id}/history/${resolvedParams.historyId}/articles?page=${page + 1}`}
                className="px-6 py-2.5 rounded-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 font-medium text-zinc-900 dark:text-zinc-100 hover:border-blue-500 dark:hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors shadow-sm"
              >
