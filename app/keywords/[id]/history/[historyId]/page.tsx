...
async function getHistoryArticles(id: string, historyId: string, skip: number = 0, limit: number = 21) {
  try {
    ...
    const artRes = await fetch(buildApiUrl(`/keywords/${id}/history/${historyId}/articles?skip=${skip}&limit=${limit}`), { cache: "no-store" });
...
export default async function HistoryArticlesPage(props: { 
  ...
  const data = await getHistoryArticles(resolvedParams.id, resolvedParams.historyId, skip, limit);
  
  if (!data) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-black">
        <p className="text-xl text-zinc-500">History or Keyword not found.</p>
      </div>
    );
  }
      ...
      <main className="max-w-6xl mx-auto px-6 pt-16 pb-32 flex flex-col items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-50 capitalize">
                {keyword.keyword} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-violet-600">Results</span>
              </h1>
              <p className="text-zinc-600 dark:text-zinc-400 mt-1">
                Articles freshly saved during this specific refresh session.
              </p>
        ...
        {articlesData.total > limit && (
          <div className="mt-16 flex items-center justify-center gap-4">
            {page > 1 ? (
              <Link
                href={`/keywords/${keyword.id}/history/${resolvedParams.historyId}?page=${page - 1}`}
